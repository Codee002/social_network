<template>
  <div class="main-mid__story">
    <div class="story-container" ref="storyContainer" @scroll="updateScrollStatus">
    ...
    <div class="story__btn-left story__scroll-btn" @click="scrollLeft" v-if="canScrollLeft">
      <i class="fa-solid fa-arrow-left"></i>
    </div>
    <div class="story__btn-right story__scroll-btn" @click="scrollRight">
...
function scrollLeft() {
  storyContainer.value.scrollBy({ left: -125, behavior: 'smooth' })
}
...
onMounted(async () => {
  try {
    let res = await axios.get(`/story/getDashboardStories`)
    stories.value = res.data.stories
    views.value = res.data.views

    // Lắng nghe khi có bạn đăng tin
    window.Echo.channel(`profile.${props.owner.id}`)
      .listen('.story.create', (e) => {
        console.log('BROADCAST NEW STORY: ', e)
        stories.value.unshift(e.story)
        views.value[e.story.id] = e.views.slice()
      })
      .error((error) => {
