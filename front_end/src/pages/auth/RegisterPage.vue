<template>
  ...
  <div class="form-group mg-form">
    <div class="form-floating mt-3 mb-3">
      ...
      />
      <i class="fa-solid fa-eye pwd-eye" @click="onHidePassword($event)"></i>
      <label class="label-input" for="password_confirm"
  ...
  <div class="form-group mg-form">
    <label for="accept"
      ...
      <a href="" class="text-decoration-none">Chính sách bảo mật</a> của chúng
      tôi</label
    >
  ...
  <p class="text-center">
    ...
    <router-link :to="{ name: 'auth.login' }"
      ><a class="text-decoration-none">Đăng nhập</a></router-link
    >
  </p>
</template>
...
function onSubmit() {
  ...
  errors.password_confirmation = validate.passwordConfirm(
    form.password,
    form.password_confirmation
  );
  if (form.check == false)
    errors.check = "Vui lòng đồng ý với điều khoản sử dụng của chúng tôi";
  else errors.check = "";
  ...
  if (hasErrors != true) {
    axios
      .post(`${config.pathApi}/auth/register`, form)
      .then((res) => {
        console.log(res);
